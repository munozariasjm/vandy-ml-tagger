...
"""
Modification of https://github.com/jet-universe/particle_transformer/blob/main/utils/convert_qg_datasets.py
"""
...
class PartData:
    ...
    def _transform(X, y, start=0, stop=-1):
        ...
        v["part_pid"] = PID
        v["part_isCHPlus"] = ((PID == 211) + (PID == 321) + (PID == 2212)).astype(
            np.float32
        )
        v["part_isCHMinus"] = ((PID == -211) + (PID == -321) + (PID == -2212)).astype(
            np.float32
        )
        v["part_isNeutralHadron"] = (
        ...
        v["part_charge"] = (
            v["part_isCHPlus"] + v["part_isEPlus"] + v["part_isMuPlus"]
